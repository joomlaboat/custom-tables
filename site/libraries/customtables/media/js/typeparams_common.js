...
function renderInput_Field_readSelectBoxes(id) {
    ...
    if (allHaveSelections) {
        ...
        const result = selectboxes.innerHTML + renderInput_Field_do(id + "_" + i, '', o2, null);
...
function renderInput_Folder(id, value, onchange) {
    ...
    const typebaseobject = document.getElementById("ct_fieldtypeeditor_box");
    const result = typebaseobject.innerHTML;
    /*
            ...
            */
    const p1 = result.indexOf('<option');
    const p2 = result.indexOf('</select>');

    let datalist = result.substring(p1, p2 - 1);
