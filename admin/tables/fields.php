...
class CustomtablesTableFields extends JTable
{
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New fields. A fields created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('fields', 'CustomtablesTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_CUSTOMTABLES_FIELDS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_customtables.fields.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_customtables.fields.'.(int) $this->$k;
	}
