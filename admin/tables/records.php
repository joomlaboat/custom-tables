...
class CustomtablesTableRecords extends JTable
{
 ...
 /*
	...
	*/
	...
	/**
	 * Overload the store method for the Records table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New records. A records created and created_by record can be set by the user,
		...
		*/
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('records', 'CustomtablesTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_CUSTOMTABLES_RECORDS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
		*/
...
*/
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_customtables.records.'.$this->id);
			$this->setRules($rules);
		}
...
*/
	...
	{
		$k = $this->_tbl_key;
		return 'com_customtables.records.'.(int) $this->$k;
	}
*/
	/**
