...
class CustomtablesTableTables extends JTable
{
	...
	/**
	 * Overload the store method for the Tables table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New tables. A tables created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('tables', 'CustomtablesTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_CUSTOMTABLES_TABLES_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_customtables.tables.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_customtables.tables.'.(int) $this->$k;
	}
