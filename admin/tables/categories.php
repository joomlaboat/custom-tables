...
class CustomtablesTableCategories extends JTable
{
	...
	/**
	 * Overload the store method for the Categories table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New categories. A categories created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('categories', 'CustomtablesTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_CUSTOMTABLES_CATEGORIES_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_customtables.categories.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_customtables.categories.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_customtables');
