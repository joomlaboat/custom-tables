...
class CustomtablesTableLayouts extends JTable
{
	...
	/**
	 * Overload the store method for the Layouts table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New layouts. A layouts created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('layouts', 'CustomtablesTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_CUSTOMTABLES_LAYOUTS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_customtables.layouts.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_customtables.layouts.'.(int) $this->$k;
	}
