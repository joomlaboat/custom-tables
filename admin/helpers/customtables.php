...
abstract class CustomtablesHelper
{
	...
	**/
	 ...
	 */
	/*
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'customtables', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
...
*/
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__customtables_'.$type.' AS a');
...
*/
	...
	public static function getActions($view,&$record = null,$views = null)
	{
		...
		foreach ($actions as $action)
		{
			...
			if (self::checkObject($record) && isset($record->id) && $record->id > 0 && !in_array($action->name,$componentActions))
			{
				...
				$permission = $user->authorise($action->name, 'com_customtables.'.$view.'.' . (int) $record->id);
				if (!$permission) // TODO removed && !is_null($permission)
				{
					if ($action->name == 'core.edit' || $action->name == $view.'.edit')
					{
						if ($user->authorise('core.edit.own', 'com_customtables.'.$view.'.' . (int) $record->id))
						{
						...
						elseif ($user->authorise($view.'edit.own', 'com_customtables.'.$view.'.' . (int) $record->id))
						{
							// If the owner matches 'me' then allow.
							if (isset($record->created_by) && $record->created_by > 0 && ($record->created_by == $user->id))
							{
						...
						elseif ($user->authorise('core.edit.own', 'com_customtables'))
						{
							// If the owner matches 'me' then allow.
							if (isset($record->created_by) && $record->created_by > 0 && ($record->created_by == $user->id))
							{
						...
						elseif ($user->authorise($view.'edit.own', 'com_customtables'))
						{
							// If the owner matches 'me' then allow.
							if (isset($record->created_by) && $record->created_by > 0 && ($record->created_by == $user->id))
							{
				...
				elseif (self::checkString($views) && isset($record->categorytable) && $record->categorytable > 0)
				{
					...
					$catpermission = $user->authorise($categoryCheck, 'com_customtables.'.$views.'.category.' . (int) $record->categorytable);
					if (!$catpermission && !is_null($catpermission))
					{
						if ($action->name == 'core.edit' || $action->name == $view.'.edit')
						{
							if ($user->authorise('core.edit.own', 'com_customtables.'.$views.'.category.' . (int) $record->categorytable))
							{
								// If the owner matches 'me' then allow.
								if (isset($record->created_by) && $record->created_by > 0 && ($record->created_by == $user->id))
								{
							...
							elseif ($user->authorise($view.'edit.own', 'com_customtables.'.$views.'.category.' . (int) $record->categorytable))
							{
								// If the owner matches 'me' then allow.
								if (isset($record->created_by) && $record->created_by > 0 && ($record->created_by == $user->id))
								{
			...
			if ($fallback)
			{
				$result->set($action->name, $user->authorise($action->name, 'com_customtables'));
			}
		}
		return $result;
	}
	...
	*/
	...
	public static function setAsset($id,$table)
	{
		...
		$parentId = $parent->id;
		$name     = 'com_customtables.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_customtables',$table);
...
*/
	...
	*	Thanks https://stackoverflow.com/a/4860432/1.8.177
	*
	*	@returns bool true on success
	**/
